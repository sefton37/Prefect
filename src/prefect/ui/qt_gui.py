...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build_minecraft_page(self) -> QtWidgets.QWidget:
        w = QtWidgets.QWidget()
        v = QtWidgets.QVBoxLayout(w)

        top = QtWidgets.QHBoxLayout()
        self.bedrock_status_label = QtWidgets.QLabel("Status: idle")
        self.bedrock_status_label.setTextInteractionFlags(QtCore.Qt.TextSelectableByMouse)
        top.addWidget(self.bedrock_status_label, 1)

        self.bedrock_start_button = QtWidgets.QPushButton("Start")
        self.bedrock_stop_button = QtWidgets.QPushButton("Stop")
        self.bedrock_stop_button.setEnabled(False)
        top.addWidget(self.bedrock_start_button)
        top.addWidget(self.bedrock_stop_button)
        v.addLayout(top)

        cmd_group = QtWidgets.QGroupBox("Console Command")
        cmd_layout = QtWidgets.QHBoxLayout(cmd_group)
        self.bedrock_cmd_input = QtWidgets.QLineEdit()
        self.bedrock_cmd_input.setPlaceholderText("Enter Bedrock server command...")
        self.bedrock_cmd_run = QtWidgets.QPushButton("Run")
        cmd_layout.addWidget(self.bedrock_cmd_input, 1)
        cmd_layout.addWidget(self.bedrock_cmd_run)
        v.addWidget(cmd_group)
        ...
        left = QtWidgets.QVBoxLayout()
        left.addWidget(QtWidgets.QLabel("Players"))
        self.bedrock_players_list = QtWidgets.QListWidget()
        left.addWidget(self.bedrock_players_list, 1)
        mid.addLayout(left, 1)

        right = QtWidgets.QVBoxLayout()
        right.addWidget(QtWidgets.QLabel("Console Output"))
        self.bedrock_console_feed = QtWidgets.QPlainTextEdit()
        self.bedrock_console_feed.setReadOnly(True)
        self.bedrock_console_feed.setMaximumBlockCount(2000)
        right.addWidget(self.bedrock_console_feed, 1)
        mid.addLayout(right, 2)
        ...
        trouble_group = QtWidgets.QGroupBox("Troubleshooting")
        t_layout = QtWidgets.QHBoxLayout(trouble_group)
        self.bedrock_kill_zombies_btn = QtWidgets.QPushButton("Kill Zombie Bedrock Processes")
        self.bedrock_kill_zombies_btn.setToolTip("Force kill any lingering 'bedrock_server' processes.")
        self.bedrock_kill_zombies_btn.setStyleSheet("background-color: #ffcccc; color: #330000;")
        t_layout.addWidget(self.bedrock_kill_zombies_btn)

        self.bedrock_update_btn = QtWidgets.QPushButton("Update Bedrock Server")
        self.bedrock_update_btn.setToolTip(
            "Download the latest Bedrock Dedicated Server (Linux) and install it into the configured server root.\n"
            "Preserves worlds/ and common config files. Stop the server before updating."
        )
        self.bedrock_update_btn.setStyleSheet("background-color: #ccffcc; color: #003300;")
        t_layout.addWidget(self.bedrock_update_btn)

        t_layout.addStretch(1)
        v.addWidget(trouble_group)

        self.bedrock_start_button.clicked.connect(self._on_bedrock_start)
        self.bedrock_stop_button.clicked.connect(self._on_bedrock_stop)
        self.bedrock_cmd_run.clicked.connect(self._run_bedrock_console_command)
        self.bedrock_cmd_input.returnPressed.connect(self._run_bedrock_console_command)
        self.bedrock_kill_zombies_btn.clicked.connect(self._on_kill_bedrock_zombies)
        self.bedrock_update_btn.clicked.connect(self._on_bedrock_update)

        return w

    def _on_bedrock_update(self) -> None:
        status = self.core.get_bedrock_status()
        if status.get("running"):
            QtWidgets.QMessageBox.warning(
                self,
                "Bedrock Server Running",
                "Stop the Bedrock server before updating.",
            )
            return

        root = str(self.settings.bedrock_server_root)
        reply = QtWidgets.QMessageBox.question(
            self,
            "Update Bedrock Server",
            "This will download the latest Minecraft Bedrock Dedicated Server (Linux) and install it into:\n\n"
            f"  {root}\n\n"
            "It will preserve:\n"
            "  - worlds/\n"
            "  - server.properties (and common JSON config files)\n\n"
            "Files that get overwritten will be backed up under prefect_backups/.\n\n"
            "Continue?",
        ...
        self._append_bedrock_activity("Starting Bedrock update...")
        self.bedrock_update_btn.setEnabled(False)
        self.bedrock_update_btn.setText("Updating...")
        self.bedrock_start_button.setEnabled(False)

        def _run() -> None:
            resp = self.core.update_bedrock_server()

            def _finish() -> None:
                self.bedrock_update_btn.setEnabled(True)
                self.bedrock_update_btn.setText("Update Bedrock Server")
                self.bedrock_start_button.setEnabled(True)
                ...
                if resp.get("ok"):
                    QtWidgets.QMessageBox.information(
                        ...
                        "Bedrock Updated",
                        "Update complete.\n\n"
                        f"Download: {resp.get('download_url', 'unknown')}\n"
                        f"Backup: {resp.get('backup_dir', 'unknown')}",
                    )
                else:
                    QtWidgets.QMessageBox.warning(
                        self,
                        "Bedrock Update Failed",
                        f"Error: {resp.get('error')}",
                    )

            # Ensure UI updates run on the Qt GUI thread (this MainWindow's thread).
            QtCore.QTimer.singleShot(0, self, _finish)

        threading.Thread(target=_run, name="prefect-bedrock-update", daemon=True).start()
    ...
    def _on_bedrock_start(self) -> None:
        logging.info("Bedrock Start button clicked")
        resp = self.core.start_bedrock_server()
        logging.info(f"start_bedrock_server response: {resp}")
        if resp.get("ok"):
            self.bedrock_start_button.setEnabled(False)
            self.bedrock_stop_button.setEnabled(True)
        else:
            self._append_bedrock_activity(f"start failed: {resp}")

    def _on_bedrock_stop(self) -> None:
        logging.info("Bedrock Stop button clicked")
        resp = self.core.stop_bedrock_server()
        logging.info(f"stop_bedrock_server response: {resp}")
        self.bedrock_start_button.setEnabled(True)
        self.bedrock_stop_button.setEnabled(False)
        if not resp.get("ok"):
            self._append_bedrock_activity(f"stop failed: {resp}")
    ...
    def _on_kill_bedrock_zombies(self) -> None:
        reply = QtWidgets.QMessageBox.question(
            self,
            "Confirm Kill",
            "This will force-kill any process matching 'bedrock_server'.\n\nContinue?",
            QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No,
        )
        if reply == QtWidgets.QMessageBox.Yes:
            resp = self.core.kill_bedrock_zombie_processes()
            self._append_bedrock_activity(f"Kill zombies: {resp}")
            QtWidgets.QMessageBox.information(self, "Result", f"Result: {resp}")
    ...
    def _run_bedrock_console_command(self) -> None:
        cmd = self.bedrock_cmd_input.text().strip()
        logging.info(f"Running Bedrock command: {cmd}")
        if not cmd:
            return
        self.bedrock_cmd_input.clear()
        self._append_bedrock_activity(f"> {cmd}")
        resp = self.core.run_bedrock_command(cmd)
        logging.info(f"run_bedrock_command response: {resp}")
        if not resp.get("ok"):
            self._append_bedrock_activity(f"Error: {resp.get('error')}")
    ...
    def _append_bedrock_activity(self, line: str) -> None:
        ts = time.time()
        self.bedrock_console_feed.appendPlainText(f"[{_fmt_time(ts)}] {line}")
    ...
    def _tick(self) -> None:
        try:
            ...
            external_running = bool(st.get("external_running"))
            if running:
                state_text = "running"
            ...
            self.status_label.setText(
                f"Status: {state_text} | players={len(players)}"
                + (f" | last_error={st.get('last_error')}" if st.get("last_error") else "")
            )

            # Button state: avoid starting a second instance if one is already running.
            self.start_button.setEnabled(not running and not external_running)
            ...
            # === Bedrock ===
            bst = self.core.get_bedrock_status()
            bplayers = bst.get("players_online", []) or []
            brunning = bool(bst.get("running"))
            bexternal_running = bool(bst.get("external_running"))
            btmux = (getattr(self.settings, "bedrock_control_mode", "managed") or "managed").lower() == "tmux"
            if brunning:
                bstate_text = "running (tmux)" if btmux else "running"
            elif bexternal_running:
                bstate_text = "running (external)"
            else:
                bstate_text = "stopped (tmux)" if btmux else "stopped"

            self.bedrock_status_label.setText(
                f"Status: {bstate_text} | players={len(bplayers)}"
                + (f" | last_error={bst.get('last_error')}" if bst.get("last_error") else "")
            )

            self.bedrock_start_button.setEnabled((not brunning) and (not bexternal_running))
            self.bedrock_stop_button.setEnabled(brunning)

            self.bedrock_players_list.clear()
            for p in sorted(bplayers):
                self.bedrock_players_list.addItem(p)

            blogs = self.core.get_bedrock_logs_since(self._last_bedrock_log_ts)
            if blogs:
                for ts, line in blogs:
                    self.bedrock_console_feed.appendPlainText(line.rstrip())
                self._last_bedrock_log_ts = max(ts for ts, _ in blogs) + 0.000001
